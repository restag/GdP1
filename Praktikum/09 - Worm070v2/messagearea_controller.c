...
{
    messagearea_t* theMessageArea = allocBlankMessageArea();
...
void eraseMessageLineInDisplay(int lineNumber)
{
    ...
    move(LINES - (MESSAGEAREA_MIN_HEIGHT - 1) + lineNumber, 0);
...
{
    mvaddstr(LINES - (MESSAGEAREA_MIN_HEIGHT - 1) + lineNumber, 1, theMessageArea -> messageLine[lineNumber] -> msgString);
}
